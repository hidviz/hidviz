...
std::string getString(const char* str, uint16_t usagePage, uint16_t usage){
    std::ostringstream os;
    os << str << " {" << std::hex << usagePage << ':' << usage << ')';
    return os.str();
}

std::string getUnknown(uint16_t usagePage, uint16_t usage){
    return getString("Unknown", usagePage, usage);
}

std::string getVendorDefined(uint16_t usagePage, uint16_t usage){
